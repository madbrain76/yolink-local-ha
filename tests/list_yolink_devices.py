#!/usr/bin/env python3
"""List YoLink devices from a local hub."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="List YoLink device ids, names, types, and models",
    )
    parser.add_argument(
        "--host",
...
def main() -> int:
    ...
    headers = ("Device ID", "Name", "Type", "Model")
    widths = [
        max(len(headers[index]), *(len(row[index]) for row in rows), 0)
    ...
    print(sep)
    for row in sorted(rows, key=lambda item: (item[2], item[1], item[0])):
